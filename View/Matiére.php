<?php
include __DIR__ . '/../Controller/LearnoraController.php';  // Chemin relatif correct

$learnoraC = new LearnoraController();
$coursList = $learnoraC->showCoursList();

foreach ($coursList as $cours) {
    echo "<div>";
    echo "<h3>" . htmlspecialchars($cours['NomC']) . "</h3>";

    // Correction pour afficher les images
    if (!empty($cours['ImageC'])) {
        // Si `ImageC` contient une chaîne binaire (base64)
        echo "<img src='data:image/jpeg;base64," . base64_encode($cours['ImageC']) . "' alt='" . htmlspecialchars($cours['NomC']) . "' style='width:200px; height:150px; object-fit:cover;'/>";
    } else {
        // Si aucune image n'est trouvée, afficher une image par défaut
        echo "<img src='Images/default.jpg' alt='Image indisponible' style='width:200px; height:150px; object-fit:cover;'/>";
    }

    echo "<p>" . htmlspecialchars($cours['DescriptionC']) . "</p>";
    echo "</div>";
}
?>
